...
use error::*;
use protos::tx_read_write_set::DataModel;
use protos::*;
...
impl RWSetBuilder {
    ...
    // get_tx_simulation_results returns the protos bytes of public rwset
    ...
    pub fn get_tx_simulation_results(&self) -> Result<TxSimulationResults> {
        // Compute the protos bytes for pub rwset
...
// TxRwSet acts as a proxy of 'rwset.TxReadWriteSet' protos message and helps constructing Read-write set specifically for KV data model
#[derive(Clone, Debug)]
pub struct TxRwSet {
...
// NsRwSet encapsulates 'kvrwset.KVRWSet' protos message for a specific name space (chaincode)
#[derive(Clone, Debug)]
pub struct NsRwSet {
...
// CollHashedRwSet encapsulates 'kvrwset.HashedRWSet' protos message for a specific collection
#[derive(Clone, Debug)]

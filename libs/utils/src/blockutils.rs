...
pub fn new_block(seq_num: u64, previous_hash: Vec<u8>) -> Block {
    Block{
        ...
        metadata: Some(BlockMetadata{ metadata: uninit_metadata()})
    }
}
...
mod tests {
    ...
    use protos::{BlockData, BlockMetadataIndex, Block};
    ...
    fn it_works() {
        ...
        block.data = Some(BlockData{data: vec![vec![1,2], vec![1,3], vec![2,3]]});
        println!("{:?}", block);
        v(&mut block);
        println!("{:?}", block);

        let meta = block.metadata.as_mut().unwrap();
        let v = meta.metadata.get_mut(BlockMetadataIndex::TransactionsFilter as usize).unwrap();
        v[0] = 2;
        v[1] = 4;
        v[2] = 5;
